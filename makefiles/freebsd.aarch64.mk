...
sys_ha = sys/+freebsd/+aarch64.ha sys/+freebsd/errno.ha sys/+freebsd/signal.ha sys/+freebsd/socket.ha sys/+freebsd/syscallno.ha sys/+freebsd/syscalls.ha sys/+freebsd/types.ha sys/u64tos.ha sys/unknown_errno.ha
$(HARECACHE)/sys.ssa: $(sys_ha)
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/sys.td.tmp -N sys $(sys_ha)

sys_s = $(HARECACHE)/sys.s sys/+freebsd/syscall+aarch64.s
$(HARECACHE)/sys.o: $(sys_s)
	@printf 'AS\t%s\n' "$@"
	@$(AS) $(ASFLAGS) -o $@ $(sys_s)

rt_ha = rt/+freebsd/env.ha rt/+freebsd/platform_abort.ha rt/+freebsd/platformstart-libc.ha rt/+freebsd/segmalloc.ha rt/abort.ha rt/ensure.ha rt/heap-libc.ha rt/initfini.ha rt/malloc.ha rt/memcpy.ha rt/memfunc_ptr.ha rt/memmove.ha rt/memset.ha rt/start.ha rt/strcmp.ha rt/u64tos.ha
$(HARECACHE)/rt.ssa: $(rt_ha) $(HARECACHE)/sys.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/rt.td.tmp -N rt $(rt_ha)

rt_s = $(HARECACHE)/rt.s rt/+freebsd/brand.s rt/+freebsd/start+aarch64-libc.s
$(HARECACHE)/rt.o: $(rt_s)
	@printf 'AS\t%s\n' "$@"
	@$(AS) $(ASFLAGS) -o $@ $(rt_s)
...
io_ha = io/+freebsd/dup.ha io/+freebsd/mmap.ha io/+freebsd/platform_file.ha io/+freebsd/sync.ha io/+freebsd/vector.ha io/arch+aarch64.ha io/copy.ha io/drain.ha io/empty.ha io/file.ha io/handle.ha io/limit.ha io/stream.ha io/tee.ha io/types.ha io/unread.ha io/util.ha io/zero.ha
...
os_ha = os/+freebsd/dirfdfs.ha os/+freebsd/exit.ha os/+freebsd/fcntl.ha os/+freebsd/fs.ha os/+freebsd/memfd.ha os/+freebsd/platform_environ.ha os/+freebsd/shm.ha os/+freebsd/status.ha os/+freebsd/stdfd.ha os/bufio.ha os/environ.ha os/os.ha
...
unix_ha = unix/+freebsd/creds.ha unix/+freebsd/nice.ha unix/+freebsd/pipe.ha unix/+freebsd/rlimit.ha unix/+freebsd/umask.ha
