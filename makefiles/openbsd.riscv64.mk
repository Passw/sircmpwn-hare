...
sys_ha = sys/+openbsd/+riscv64.ha sys/+openbsd/dynamic_linker.ha sys/+openbsd/errno.ha sys/+openbsd/libc.ha sys/+openbsd/signal.ha sys/+openbsd/socket.ha sys/+openbsd/syscalls.ha sys/+openbsd/types.ha sys/u64tos.ha sys/unknown_errno.ha
$(HARECACHE)/sys.ssa: $(sys_ha)
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/sys.td.tmp -N sys $(sys_ha)

rt_ha = rt/+openbsd/env.ha rt/+openbsd/platform_abort.ha rt/abort.ha rt/ensure.ha rt/initfini.ha rt/malloc+libc.ha rt/memcpy.ha rt/memfunc_ptr.ha rt/memmove.ha rt/memset.ha rt/start+libc.ha rt/strcmp.ha rt/u64tos.ha
...
io_ha = io/+openbsd/dup.ha io/+openbsd/mmap.ha io/+openbsd/platform_file.ha io/+openbsd/sync.ha io/+openbsd/vector.ha io/arch+riscv64.ha io/copy.ha io/drain.ha io/empty.ha io/file.ha io/handle.ha io/limit.ha io/stream.ha io/tee.ha io/types.ha io/unread.ha io/util.ha io/zero.ha
...
time_ha = time/+openbsd/functions.ha time/arithm.ha time/duration.ha time/instant.ha
...
os_ha = os/+openbsd/dirfdfs.ha os/+openbsd/exit.ha os/+openbsd/fcntl.ha os/+openbsd/fs.ha os/+openbsd/memory.ha os/+openbsd/platform_environ.ha os/+openbsd/shm.ha os/+openbsd/status.ha os/+openbsd/stdfd.ha os/bufio.ha os/environ.ha os/os.ha
...
unix_ha = unix/+openbsd/creds.ha unix/+openbsd/nice.ha unix/+openbsd/pipe.ha unix/+openbsd/rlimit.ha unix/+openbsd/umask.ha
...
$(HARECACHE)/unix_signal.ssa: $(unix_signal_ha) $(HARECACHE)/errors.td $(HARECACHE)/sys.td $(HARECACHE)/unix.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
...
unix_tty_ha = unix/tty/+openbsd/isatty.ha unix/tty/+openbsd/open.ha unix/tty/+openbsd/pgid.ha unix/tty/+openbsd/pty.ha unix/tty/+openbsd/termios.ha unix/tty/+openbsd/winsize.ha unix/tty/types.ha
