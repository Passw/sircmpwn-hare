...
io_ha = io/+dragonfly/dup.ha io/+dragonfly/mmap.ha io/+dragonfly/platform_file.ha io/+dragonfly/sync.ha io/+dragonfly/vector.ha io/arch+x86_64.ha io/copy.ha io/drain.ha io/empty.ha io/file.ha io/handle.ha io/limit.ha io/stream.ha io/tee.ha io/types.ha io/unread.ha io/util.ha io/zero.ha
...
path_ha = path/+dragonfly.ha path/buffer.ha path/error.ha path/ext_stack.ha path/iter.ha path/posix.ha path/prefix.ha path/stack.ha
...
time_ha = time/+dragonfly/functions.ha time/arithm.ha time/duration.ha time/instant.ha
...
os_ha = os/+dragonfly/dirfdfs.ha os/+dragonfly/exit.ha os/+dragonfly/fcntl.ha os/+dragonfly/fs.ha os/+dragonfly/platform_environ.ha os/+dragonfly/shm.ha os/+dragonfly/status.ha os/+dragonfly/stdfd.ha os/bufio.ha os/environ.ha os/os.ha
$(HARECACHE)/os.ssa: $(os_ha) $(HARECACHE)/bufio.td $(HARECACHE)/errors.td $(HARECACHE)/fs.td $(HARECACHE)/io.td $(HARECACHE)/path.td $(HARECACHE)/rt.td $(HARECACHE)/strings.td $(HARECACHE)/sys.td $(HARECACHE)/time.td $(HARECACHE)/types_c.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
...
unix_ha = unix/+dragonfly/creds.ha unix/+dragonfly/nice.ha unix/+dragonfly/pipe.ha unix/+dragonfly/rlimit.ha unix/+dragonfly/umask.ha
...
unix_signal_ha = unix/signal/+dragonfly.ha unix/signal/types.ha
$(HARECACHE)/unix_signal.ssa: $(unix_signal_ha) $(HARECACHE)/errors.td $(HARECACHE)/sys.td $(HARECACHE)/time.td $(HARECACHE)/unix.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/unix_signal.td.tmp -N unix::signal $(unix_signal_ha)

os_exec_ha = os/exec/+dragonfly/exec.ha os/exec/+dragonfly/platform_cmd.ha os/exec/+dragonfly/process.ha os/exec/cmd.ha os/exec/types.ha
...
$(HARECACHE)/unix_tty.ssa: $(unix_tty_ha) $(HARECACHE)/errors.td $(HARECACHE)/fmt.td $(HARECACHE)/fs.td $(HARECACHE)/io.td $(HARECACHE)/os.td $(HARECACHE)/sys.td $(HARECACHE)/types_c.td $(HARECACHE)/unix.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
